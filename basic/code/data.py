...
def perturb(image, keep, val_rangle=(0,255), angle_limit=15, scale_limit=0.1, translate_limit=3, distort_limit=3, illumin_limit=0.7):
    ...
    if u>keep :
        ...
        if 1:
            ...
            perturb *= alpha
            perturb = np.clip(perturb,val_rangle[0],val_rangle[1])
            pass
...
def make_perturb_images(images, keep,  val_rangle=(0,255) ):
    arguments = np.zeros(images.shape)
    for n in range(len(images)):
        arguments[n] = perturb(images[n],keep=keep, val_rangle=val_rangle)
